#!/usr/bin/env python3
"""Import the animated effect sheets under art/sprites into the particle system.

The packs in art/sprites/shaders are laid out the way effect packs always
...
Output:
    engine/assets/particles/textures/sheets/*.png   the trimmed sheets
    engine/assets/particles/sprite_sheets.toml      one [texture.*] per animation
...
SRC = os.path.join(REPO, "art", "sprites", "shaders")
OUT_DIR = os.path.join(REPO, "engine", "assets", "particles", "textures", "sheets")
OUT_TOML = os.path.join(REPO, "engine", "assets", "particles", "sprite_sheets.toml")
...
def main() -> int:
    ...
    header = (
        ...
        "# art/sprites/shaders. Several entries share one sheet: the\n"
