...
int main()
{
    ...
    }
    {
        ...
        absurd.uiScale = 20.0f;
        require(sanitised(absurd).autosaveSeconds == EditorSettings::kMaxSeconds,
                 "and an hour and a half is not a backup either");
        require(sanitised(absurd).uiScale == EditorSettings::kMaxUiScale,
                "interface scale is clamped to a readable supported range");
    }
