...
int main()
{
    ...
    // --- saved marker -------------------------------------------------------
    {
        ...
        history.markSaved();
        history.undo(doc);
        history.run(doc, makeCreateEntity(make("different_branch")));
        require(!history.savedStateReached(),
                "a different edit at the saved depth remains dirty");
    }

    // --- live inspector transaction ----------------------------------------
    {
        Doc doc;
        CommandStack history;
        doc.add(make("inspected"));
        EntityEditTransaction edit;
        edit.begin(*doc.find("inspected"));
        doc.find("inspected")->transform.position.x = 2.0f;
        doc.touch();
        doc.find("inspected")->transform.position.x = 7.0f;
        doc.touch();
        std::optional<Command> command =
            edit.commit(doc, "edit inspected transform");
        require(command.has_value(), "a live edit closes as one command");
        history.run(doc, std::move(*command));
        require(history.undo(doc), "the live edit can be undone");
        require(doc.find("inspected")->transform.position.x == 0.0f,
                "undo restores the value from interaction start, not the last "
                "drag frame");
        require(history.redo(doc), "the live edit can be redone");
        require(doc.find("inspected")->transform.position.x == 7.0f,
                "redo restores the final live value");

        edit.begin(*doc.find("inspected"));
        doc.find("inspected")->transform.position.x = 11.0f;
        require(edit.cancel(doc), "an active edit can be cancelled");
        require(doc.find("inspected")->transform.position.x == 7.0f,
                "cancel restores its activation-time snapshot");
    }
