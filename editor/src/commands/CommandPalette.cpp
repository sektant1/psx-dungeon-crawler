                        ...
                        const std::vector<PaletteAction>& actions)
{
    ...
    const float width =
        std::min(720.0f, std::max(220.0f, viewport->WorkSize.x - 24.0f));
    ...
    if (ImGui::BeginPopup("##palette",
        ...
        const bool queryChanged = ImGui::InputTextWithHint(
            "##query", "Type a command", state.query, sizeof(state.query));
        ...
        const auto enabledAt = [&](int row) {
            return row >= 0 && row < count &&
                   actions[matches[std::size_t(row)].index].enabled;
        };
        const auto firstEnabled = [&] {
            for (int row = 0; row < count; ++row)
                if (enabledAt(row)) return row;
            return 0;
        };
        bool highlightChanged = queryChanged;
        if (queryChanged) {
            state.highlighted = firstEnabled();
            state.message.clear();
        }
        const bool moveDown = ImGui::IsKeyPressed(ImGuiKey_DownArrow, true) ||
                              ImGui::IsKeyPressed(ImGuiKey_GamepadDpadDown, true);
        const bool moveUp = ImGui::IsKeyPressed(ImGuiKey_UpArrow, true) ||
                            ImGui::IsKeyPressed(ImGuiKey_GamepadDpadUp, true);
        if (count > 0 && (moveDown || moveUp)) {
            const int direction = moveDown ? 1 : -1;
            int next = state.highlighted;
        ...
        const float maxResultsHeight =
            std::max(rowHeight, viewport->WorkSize.y * 0.62f - 72.0f);
        const float rows = std::min(
            float(std::max(count, 1)),
            std::max(1.0f, std::floor(maxResultsHeight / rowHeight)));
        if (ImGui::BeginChild("##results", ImVec2(0.0f, rows * rowHeight))) {
            for (int i = 0; i < count; ++i) {
                ...
                // bottom of the list.
                if (highlighted && highlightChanged)
                ...
                const float shortcutWidth =
                    action.shortcut.empty()
                        ? 0.0f
                        : ImGui::CalcTextSize(action.shortcut.c_str()).x + 20.0f;
                ImGui::PushClipRect(
                    rowMin, ImVec2(std::max(rowMin.x, rowMax.x - shortcutWidth),
                                   rowMax.y),
                    true);
        ...
        if (chosen >= 0 && chosen < count) {
            ...
            state.message = action.detail.empty()
