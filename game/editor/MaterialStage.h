...
public:
    ...
    // being a reference.
    eng::NodeHandle gizmoTarget() const { return mSubject; }
...
private:
    void buildSphereRig(eng::Renderer& renderer);
    void buildQuadRig(eng::Renderer& renderer);
    void destroyStage(eng::Renderer& renderer);   // big viewport rig only
    void buildThumbnailRig(eng::Renderer& renderer);
    void destroyThumbnailRig(eng::Renderer& renderer);

    StagePreviewCatalog mCatalog;
    bool mCatalogLoaded = false;

    StagePreview mMode = StagePreview::Sphere;
    eng::NodeHandle mSubject;
