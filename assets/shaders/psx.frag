...
//
// The fresnel rim and the alpha cutout are NOT variants: `rimColour`,
// `rimPower` and `alphaScissor` are declared by every variant and neutral at
// their defaults, so eng::ecs::ShaderParams can drive any material without the
// author first swapping to a differently-named shader. Rim still needs LIT --
// it is reflected light, and an unlit pass has none to reflect.
...
#endif
// Declared by every variant, not just the ones named after them, so that
// eng::ecs::ShaderParams can drive any material without the author first
// swapping to a differently-named shader. Both are neutral at their defaults --
// `alphaScissor` 0 discards nothing, `rimColour.a` 0 adds nothing -- so a
// material that never sets them renders exactly as it did before they existed.
...
void main()
{
    ...
    // Alpha is always carried now, not only under the blend/scissor variants:
    // a universal `alphaScissor` needs something to compare against on every
    // material. An opaque pass does not blend, so writing a real alpha into the
    // colour target changes nothing that is read -- the value only decides
    // whether the cutout below discards.
#ifdef NO_TEXTURE
    ...
    // Unconditional, because the threshold is a uniform now rather than a
    // variant. The cost is that every material becomes "may discard", which
    // gives up early-Z; at this renderer's resolution the frame is dominated by
    // the post chain, and it was measured before this was made universal.
