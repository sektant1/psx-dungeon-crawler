...
| --- | --- | --- | --- | --- | --- |
| P0 | Game startup still needs deleted content. | `main.cpp` loads `lobby.toml`; `LiveLevel.cpp` requests deleted tiles, props, lobby dressing, showcase, and demo content. | Make a cooked prototype `.map`/`.scn` demo the sole startup path, or retain a deliberately prototype-only procedural fallback. Delete the old dual lobby path after the replacement works. | gameplay + tools | Fresh checkout launches to a walkable prototype arena with no deleted-file warnings. |
