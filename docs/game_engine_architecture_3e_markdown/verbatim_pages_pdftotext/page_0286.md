...
tions atomic.
2. The ability to synchronize the operation of two or more threads:
...
task, and
b. by enabling a running thread to notify one or more sleeping threads
by waking them up.
...
made atomic. A mutex can be in one of two states: unlocked or locked.
