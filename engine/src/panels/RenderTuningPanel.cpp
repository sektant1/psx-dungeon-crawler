...
void RenderTuningPanel::init(Renderer* r)
{
    mR = r;
...
void RenderTuningPanel::drawStats(const Context& ctx)
{
    mCore = ctx.core();
    ...
    mCore->frameStats(batches, tris);
    ImGui::Text("batches %zu  triangles %zu", batches, tris);
    const Ogre::Vector3 p = mCore->camera()->getDerivedPosition();
    ImGui::Text("window %ux%u  cam %.2f %.2f %.2f", mCore->window()->getWidth(),
                mCore->window()->getHeight(), p.x, p.y, p.z);
}

void RenderTuningPanel::drawShaders()
{
    const EnvState& env = mR->envState();
    bool wireframe = env.wireframe;
    if (ImGui::Checkbox("wireframe view", &wireframe)) {
        mR->setWireframeDebug(wireframe);
        // Enabling forces pixelSize 1; reapply the chosen line thickness.
        if (wireframe && wireThickness > 1)
            mR->setPixelSize(wireThickness);
    }
    ...
    if (wireframe) {
        if (ImGui::ColorEdit3("wire colour", &wireColor.x))
            mR->setMaterialParam("PSX/DebugWireframe", "wireColor",
                                 glm::vec4(wireColor, 1.0f));
        if (ImGui::SliderFloat("wire depth fade", &wireDepthFade, 0.0f, 0.5f,
                               "%.3f"))
            mR->setMaterialParam("PSX/DebugWireframe", "wireDepthFade",
                                 wireDepthFade);
...
void RenderTuningPanel::drawPixelArt()
{
    ...
    if (ImGui::Button("Reset stylize")) {
        ...
        mR->setMaterialParam("PSX/PixelStylize", "shadowsEnabled", 1.0f);
        mR->setMaterialParam("PSX/PixelStylize", "highlightsEnabled", 1.0f);
        mR->setMaterialParam("PSX/PixelStylize", "outlineEnabled", 1.0f);
        mR->setMaterialParam("PSX/PixelStylize", "shadowStrength", inkStrength);
        mR->setMaterialParam("PSX/PixelStylize", "shadowThreshold", inkThreshold);
        mR->setMaterialParam("PSX/PixelStylize", "shadowColor", inkColor);
        mR->setMaterialParam("PSX/PixelStylize", "highlightStrength", highlightStrength);
        mR->setMaterialParam("PSX/PixelStylize", "highlightThreshold", highlightThreshold);
        mR->setMaterialParam("PSX/PixelStylize", "highlightDarkFade", highlightDarkFade);
        mR->setMaterialParam("PSX/PixelStylize", "highlightColor", highlightColor);
        mR->setMaterialParam("PSX/PixelStylize", "outlineOpacity", outlineOpacity);
        mR->setMaterialParam("PSX/PixelStylize", "outlineThickness", outlineThickness);
        mR->setMaterialParam("PSX/PixelStylize", "outlineDepthSens", outlineDepthSensitivity);
        mR->setMaterialParam("PSX/PixelStylize", "outlineNormalSens", outlineNormalSensitivity);
        mR->setMaterialParam("PSX/PixelStylize", "outlineSharpness", outlineSharpness);
        mR->setMaterialParam("PSX/PixelStylize", "outlineDistFade", outlineDistanceFade);
        mR->setMaterialParam("PSX/PixelStylize", "outlineDarkFade", outlineDarkFade);
        mR->setMaterialParam("PSX/PixelStylize", "outlineColor", outlineColor);
    }
    if (ImGui::Checkbox("ink shadows", &inkEnabled))
        mR->setMaterialParam("PSX/PixelStylize", "shadowsEnabled",
                             inkEnabled ? 1.0f : 0.0f);
    ImGui::SameLine();
    if (ImGui::Checkbox("highlights", &highlightsEnabled))
        mR->setMaterialParam("PSX/PixelStylize", "highlightsEnabled",
                             highlightsEnabled ? 1.0f : 0.0f);
    ImGui::SameLine();
    if (ImGui::Checkbox("outlines", &outlinesEnabled))
        mR->setMaterialParam("PSX/PixelStylize", "outlineEnabled",
                             outlinesEnabled ? 1.0f : 0.0f);
    if (ImGui::SliderFloat("ink strength", &inkStrength, 0.0f, 1.0f, "%.2f"))
        mR->setMaterialParam("PSX/PixelStylize", "shadowStrength",
                             inkStrength);
    if (ImGui::SliderFloat("ink edge threshold", &inkThreshold, 0.0f, 1.0f,
                           "%.2f"))
        mR->setMaterialParam("PSX/PixelStylize", "shadowThreshold",
                             inkThreshold);
    ...
    if (ImGui::SliderFloat("highlight strength", &highlightStrength, 0.0f,
                           0.5f, "%.2f"))
        mR->setMaterialParam("PSX/PixelStylize", "highlightStrength",
                             highlightStrength);
    if (ImGui::SliderFloat("highlight threshold", &highlightThreshold, 0.0f,
                           1.5f, "%.2f"))
        mR->setMaterialParam("PSX/PixelStylize", "highlightThreshold",
                             highlightThreshold);
    ...
    if (bloomChanged)
        mR->setBloomParams(bloomThreshold, bloomIntensity);
    if (ImGui::Checkbox("vignette", &vignetteEnabled))
        mR->setMaterialParam("PSX/DitherPost", "vignetteStrength",
    ...
    if (ImGui::ColorEdit3("vignette colour", &vignetteColor.x))
        mR->setMaterialParam("PSX/DitherPost", "vignetteColor",
                             vignetteColor);
    ...
    if (gradeChanged) {
        renderPreset = 0;
        mR->setGradeParams(gDesat, gContrast, gShadow, gMid);
    }
    bool paletteChanged =
    ...
    if (paletteChanged) {
        renderPreset = 0;
        mR->setMaterialParam("PSX/DitherPost", "gradeSaturation",
                             gradeSaturation);
        mR->setMaterialParam("PSX/DitherPost", "gradeTintStrength",
                             gradeTintStrength);
        mR->setMaterialParam("PSX/DitherPost", "gradeBlackLift",
                             gradeBlackLift);
    }
}

void RenderTuningPanel::drawCamera()
{
    const EnvState& env = mR->envState();
...
void RenderTuningPanel::copyToml()
{
    const EnvState& env = mR->envState();
    char buf[4096];
...
void RenderTuningPanel::drawAll(const Context& ctx)
{
    if (ImGui::CollapsingHeader("Stats", ImGuiTreeNodeFlags_DefaultOpen)) drawStats(ctx);
    if (ImGui::CollapsingHeader("PSX Shaders")) drawShaders();
    if (ImGui::CollapsingHeader("Pixel Art", ImGuiTreeNodeFlags_DefaultOpen)) drawPixelArt();
    if (ImGui::CollapsingHeader("Camera")) drawCamera();
    if (ImGui::Button("Copy all as TOML")) copyToml();
}
