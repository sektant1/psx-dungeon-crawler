...
// Tint and hue-drift axis of the enchantment shader. Which schools of magic
// exist, and what colour each one is, is a game decision: the engine takes the
// colours it is given.
struct EnchantmentPalette {
    glm::vec4 colour{0.62f, 0.32f, 1.0f, 0.68f};
    glm::vec3 scrollDirection{1.0f, 1.0f, 1.0f};
};

struct EnchantmentDesc {
    EnchantmentPalette palette;
    float strength = 1.0f;
...
// A non-finite colour channel reaches the shader as a NaN uniform and paints
// the mesh black or transparent, so replace each bad channel individually
// rather than discarding a palette that is only partly wrong.
inline EnchantmentPalette sanitizeEnchantmentPalette(EnchantmentPalette palette)
{
    const EnchantmentPalette defaults;
    for (int channel = 0; channel < 4; ++channel)
        palette.colour[channel] =
            std::isfinite(palette.colour[channel])
                ? std::clamp(palette.colour[channel], -4.0f, 4.0f)
                : defaults.colour[channel];
    for (int axis = 0; axis < 3; ++axis)
        palette.scrollDirection[axis] =
            std::isfinite(palette.scrollDirection[axis])
                ? std::clamp(palette.scrollDirection[axis], -4.0f, 4.0f)
                : defaults.scrollDirection[axis];
    return palette;
}
...
inline EnchantmentDesc sanitizeEnchantmentDesc(const EnchantmentDesc& desc)
{
    ...
    clean.palette = sanitizeEnchantmentPalette(clean.palette);
