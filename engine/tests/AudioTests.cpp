...
int main()
{
    ...
    PlaybackSettings s;
    require(s.gainDb == 0.0f && s.pitch == 1.0f && !s.loop,
            "settings defaults");
    require(s.bus == AudioBus::Sfx && !s.spatialized && !s.streaming,
            "settings routing defaults");
    require(std::abs(decibelsToLinear(-6.0206f) - 0.5f) < 0.001f,
            "decibels convert to linear amplitude");
    require(std::abs(linearToDecibels(0.5f) + 6.0206f) < 0.001f,
            "linear amplitude converts to decibels");
    require(linearToDecibels(0.0f) == -80.0f, "zero maps to silence");
    ...
    FileSystem::fileWrite(
        wav, "RIFFxxxxWAVE"); // stub; existence is all load() checks
    ...
    PlaybackSettings loud;
    loud.gainDb = -6.0f;
    loud.loop = true;
    loud.bus = AudioBus::Weapons;
    loud.spatialized = true;
    loud.position = {2.0f, 1.0f, -4.0f};
    ...
    inst->setGainDb(-9.0f);
    inst->setPitch(1.5f);
    inst->setPosition({1.0f, 0.0f, -2.0f});
    inst->setVelocity({0.0f, 0.0f, 1.0f});
    require(audio.activeCount() == 1 &&
                audio.activeCount(AudioBus::Weapons) == 1,
            "one active instance on weapons bus");
    inst->pause();
    audio.update(0.016f);
    require(inst->paused() && audio.activeCount() == 1,
            "paused voice remains retained");
    inst->resume();
    require(!inst->paused(), "voice resumes");
    inst->stop();
    require(!inst->isPlaying() && inst->finished(),
            "stopped instance is finished");

    // one-shot: fire and forget, no handle retained
    require(audio.playOneShot(realWav, PlaybackSettings{}), "one-shot plays");

    audio.setBusGainDb(AudioBus::Music, -8.0f);
    audio.setBusDuckDb(AudioBus::Music, -10.0f, 0.01f, 0.5f);
    audio.setBusVolume(AudioBus::Ui, 0.5f);
    ...
    require(audio.busGainDb(AudioBus::Music) == -8.0f, "bus dB retained");
    require(audio.sampleRate() > 0 && audio.maxVoices() >= 8,
            "backend exposes timing and voice budget");
    const AudioStats stats = audio.stats();
    require(stats.voicesStarted >= 2 && stats.voiceLimit == audio.maxVoices(),
            "voice telemetry reported");
    audio.terminate();
